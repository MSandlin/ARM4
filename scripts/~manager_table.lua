...
function onTableRoll(rSource, rTarget, rRoll)
	...
	for _,v in ipairs(aResults) do
		if v.sText ~= "" then
			table.insert(aResultText, v.sText);
			
			local sTable = string.match(v.sText, "^%[(.+)%]$");
			if sTable then
				local nodeTable = findTable(sTable);
				if nodeTable then
					table.insert(aCascade, nodeTable);
				else
		...
		if v.sClass and v.sClass ~= "" then
			if v.sClass == "table" then
				table.insert(aCascade, DB.findNode(v.sRecord));
			else
	...
	for _,vTable in ipairs(aCascade) do
		local rTableRoll = {};
		rTableRoll.nodeTable = vTable;
