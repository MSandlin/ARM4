...
function onInit()
	...
	Comm.registerSlashHandler("ivanna", processIvannaEyes);
...
-- Horizontal rule
function processHorizontalRule(cmd,params)
	local msg = {};
	msg.font = "systemfont";
	msg.text = "";
	msg.icon = "hrbar";
...
function processIvannaEyes(cmd,params)
	local msg = {};
	msg.font = "systemfont";
	msg.text = "";
...
function onSlashCommand(command, parameters)
	...
	if User.isHost() then
		...
		SystemMessage("/hr  |Shows a Footer");
		...
		SystemMessage("/next : Shows you want to talk next");
